...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    ...
    await request(server).get('/api/azure-dev-ops/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/azure-dev-ops/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/azure-dev-ops/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/azure-dev-ops/todos/${createdTodoItem.id}`)
